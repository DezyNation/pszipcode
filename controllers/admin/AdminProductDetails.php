...
include_once(_PS_MODULE_DIR_.'productavailabilitycheckbyzipcode/classes/ProductDetails.php');
class AdminProductDetailsController extends ModuleAdminControllerCore
{
    /**
     * Function responsible to Creating list of all the available products for specific zone
     * @date 14-02-2023
     * @author 
    ...
    public function __construct()
    {
        ...
        if ($zone_id != null) {
            $this->context->cookie->__set('zone_cookie1', $zone_id);
            $this->context->cookie->__set('productdetailskb_pacbz_product_zone_mappingFilter_a!product_id', null);
            $this->context->cookie->__set('productdetailskb_pacbz_product_zone_mappingFilter_p!product_name', null);
        } else {
            $zone_id = $this->context->cookie->zone_cookie1;
        }
        if (($zone_id === false) || (Tools::isEmpty($zone_id))) {
            $linkobj = new Link();
            $red_path = $linkobj->getAdminLink('AdminProductMap');
        ...
        $this->table = 'kb_pacbz_product_zone_mapping';
        $this->className = 'ProductDetails';
        $this->show_toolbar = true;
        $this->list_no_link = true;
        $this->bootstrap = true;
        $this->_select = 'a.`product_id` as pid,a.`zone_id` as zid,p.`product_name` as pname,'.
            'z.`zone_name` as zname';
        $this->_join = 'INNER JOIN `'. _DB_PREFIX_ .'kb_pacbz_products` p ON a.product_id = p.id_kb_pacbz_products ' .
            'INNER JOIN `' . _DB_PREFIX_ . 'kb_pacbz_zones` z ON a.zone_id = z.id_kb_pacbz_zones';
        $this->_where = 'AND a.zone_id ="' .pSQL($zone_id).'"';
        ...
        $this->fields_list = array(
            'zid' => array(
                'title' => $module->l('Zone ID', 'AdminProductDetails'),
                'align' => 'center',
                'search' => false
            ),
            'zname' => array(
                'title' => $module->l('Zone', 'AdminProductDetails'),
                'align' => 'center',
                'search' => false
            ),
            'pid' => array(
                'title' => $module->l('Product ID', 'AdminProductDetails'),
                'align' => 'center',
                'filter_key' => 'a!product_id',
                'class' => 'fixed-width-lg',
            ),
            'pname' => array(
                'title' => $module->l('Product', 'AdminProductDetails'),
                'align' => 'center',
                'filter_key' => 'p!product_name',
            ),
        );
        
        if (Tools::getValue('success_delete') == 1) {
            $this->confirmations = $module->l('Product succesfully Deleted.', 'AdminProductDetails');
        }
        
        $this->bulk_actions = array(
            'delete' => array(
                'text' => $module->l('Delete selected', 'AdminProductDetails'),
                'confirm' => $module->l('Delete selected items?', 'AdminProductDetails'),
                'icon' => 'icon-trash'
            )
        );
    }
    ...
    /**
     * Function responsible to add "Delete" button for the list generated from the construct function.
     * @date 14-02-2023
     * @author 
     * @commenter Vishal Goyal
     */
    ...
    /**
     * Function responsible to Add the "Add new" and "Back" button on the Product Details admin page
     * @date 14-02-2023
     * @author 
     * @commenter Vishal Goyal
     */
    ...
    {
        $module = Module::getInstanceByName('productavailabilitycheckbyzipcode');
        //Function to display add & back button in same controller
        if (empty($this->display)) {
            $this->page_header_toolbar_btn['kb_pacbz_product_zone_mapping'] = array(
                'href' => 'index.php?controller=AdminProductMap&token=' .
                    Tools::getAdminTokenLite('AdminProductMap'),
                //'desc' => $module->l('Back', null, null, false),
                'desc' => $module->l('Back', 'AdminProductDetails'),
                'icon' => 'process-icon-back'
            );
        }
        
        $redirect_href = 'index.php?controller=AdminProductDetails';
        if (empty($this->display)) {
            $this->page_header_toolbar_btn['addkb_pacbz_product_zone_mapping'] = array(
                'href' => $redirect_href.'&addkb_pacbz_product_zone_mapping&token='.$this->token,
                //'desc' => $module->l('Add new', null, null, false),
                'desc' => $module->l('Add new', 'AdminProductDetails'),
    ...
    /**
     * Function responsible to Add the "Back" button on the "Product Details" admin page
     * @date 14-02-2023
     * @author 
     * @commenter Vishal Goyal
     */
    public function initPageHeaderToolbarNew()
    {
        ...
        $this->page_header_toolbar_btn['kb_pacbz_product_zone_mapping'] = array(
            'href' => 'index.php?controller=AdminProductDetails&token=' .
                Tools::getAdminTokenLite('AdminProductDetails'),
            //'desc' => $module->l('Back', null, null, false),
            'desc' => $module->l('Back', 'AdminProductDetails'),
            'icon' => 'process-icon-back'
        );
    ...
    /**
     * Function responsible for Creating configuration setting form for "Product Details" page
     * @date 14-02-2023
     * @author 
     * @commenter Vishal Goyal
     */
    public function renderForm()
    {
        ...
        $this->page_header_toolbar_title = $module->l('Product-Zone Mapping', 'AdminProductDetails');
        ...
        $formvalue = json_decode($config, true);//d($formvalue);
        ...
        $upload_csv = array(
            'label' => $module->l('Upload CSV File', 'AdminProductDetails'),
            'type' => 'switch',
            'hint' => $module->l('Upload CSV File to add large number of zip-codes.', 'AdminProductDetails'),
...
//        }
        ...
        $this->fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => $module->l('Add new', 'AdminProductDetails'),
                ),
                'input' => array(
                    $upload_csv,
//                    array(
//                        'type' => 'select',
//                        'label' => $module->l('Search Product', 'AdminProductDetails'),
//                        'name' => 'product_availability_check_by_zipcode[search_product][]',
//                        'multiple' => 'multiple',
//                        'hint' => $module->l('Select Chargable Payment Method', 'AdminProductDetails'),
//                        'is_bool' => true,
//                        'required' => true,
//                        'options' => array(
//                            'query'=> $option1,
...
//                    ),
                    ...
                    array(
                        'type' => 'text',
                        'label' => $module->l('Search Product', 'AdminProductDetails'), // The <label> for this <select> tag.
                        'hint' => $module->l('Select products to map with zone', 'AdminProductDetails'),
                    ...
                    array(
                        'type' => 'select',
                        'multiple' =>true,
                        'label' => $module->l('Select Zone', 'AdminProductDetails'),
                        'name' => 'product_availability_check_by_zipcode[select_zone][]',
                        'hint' => $module->l('Enter define the zone name.', 'AdminProductDetails'),
                    ...
                    array(
                        'type' => 'file',
                        'label' => $module->l('Choose CSV File', 'AdminProductDetails'),
                        'name' => 'product_availability_check_by_zipcode[selected_file]',
                        'id' => 'uploadedfile',
                        'required' => true,
                        'display_image' => true,
                        'desc' => $module->l('Download Sample of CSV File', 'AdminProductDetails'),
                        'hint' => $module->l('Select the CSV File to be add zipcodes.', 'AdminProductDetails'),
                    ),
                ),
                'submit' => array(
                    'title' => $module->l('Save', 'AdminProductDetails'),
        ...
        $action = 'index.php?controller=AdminProductDetails&addkb_pacbz_product_zone_mapping&token='.$this->token;
        $helper->name_controller = $this->controller_name;
        $helper->languages = $languages;
        $helper->default_form_language = $this->context->language->id;
        $helper->currentIndex = 'index.php?controller=AdminProductDetails&addkb_pacbz_product_zone_mapping&token=' .
            Tools::getAdminTokenLite('AdminProductDetails');
    ...
    public function initProcess()
    {
        ...
        if (Tools::isSubmit('product_availability_check_by_zipcode')) {
            /**
             * Code responsible for Validating the "Add new" page form
             * @date 14-02-2023
             * @author 
             * @commenter Vishal Goyal
             */
            ...
            if ($formvalue['upload_csv'] == 0) {
                if (!isset($formvalue['search_product'])) {
                    $this->errors[] = $module->l('Please search product to map with zone.', 'AdminProductDetails');
                    $error_count++;
                }
                
                if (!isset($formvalue['select_zone'])) {
                    $this->errors[] = $module->l('Please select zone to map with the product.', 'AdminProductDetails');
                    $error_count++;
                }
            } else {
                $product_zipcode_array = Tools::getvalue('product_availability_check_by_zipcode');
                ...
                if ($formvalue['selected_file'] == null) {
                    $this->errors[] .= $module->l('Please upload the CSV file.', 'AdminProductDetails');
                    $error_count++;
                } else {
                    if ($_FILES['product_availability_check_by_zipcode']['name']['selected_file'] != null) {
                        ...
                        if (in_array($file_ext, $expensions) === false) {
                            $this->errors[] .= $module->l('Please choose file with .csv extension.', 'AdminProductDetails');
                            $error_count++;
                        }

                        if ($file_size >= 4194304) {
                            $this->errors[] .= $module->l('File size must be less than 4 MB.', 'AdminProductDetails');
                            $error_count++;
                        }
                        ...
                        if ($error_count == 0) {
                            ...
                            if ($this->validateCSVFile($csv_file_array) == false) {
                                ...
                                if ($this->csv_invalid == 1) {
                                    $this->errors[] .= $module->l('Invalid content inside CSV file.Please check the sample document.', 'AdminProductDetails');
                                } else {
                                    //d($this->csv_line_number);
                                    $pv_part1 = $module->l('Please check ', 'AdminProductDetails');
                                    $pv_part2 = $module->l('th row of CSV File.Following are the validation rules-', 'AdminProductDetails');
                                    $pv_middle = implode(",", array_unique($this->csv_line_number));
                                    $this->errors[] .= $pv_part1 ." ". $pv_middle . $pv_part2;
                                    
                                    $product_table1 = $module->l('Product Id must exist.', 'AdminProductDetails');
                                                                   
                                    $zone_table1 = $module->l('Zone Id must exist.', 'AdminProductDetails');
            ...
            if ($error_count == 0) {
                ...
                $redirect_path = 'index.php?controller=AdminProductMap&success_add=1&token=';
                Tools::redirectAdmin($redirect_path .Tools::getAdminTokenLite('AdminProductMap'));
            }
        }
        parent::initProcess();
    }
    ...
    /**
     * Function responsible to delete the existing product-zone Mapping, triggered when we click the delete button on the "Product Details" page
     * @date 14-02-2023
     * @author 
     * @commenter Vishal Goyal
     */
    public function processDelete()
    {
        ...
        $product_id = Tools::getValue('id_kb_pacbz_product_zone_mapping');
        if (isset($_REQUEST['deletekb_pacbz_product_zone_mapping'])) {
            $sql1 = 'DELETE FROM `'._DB_PREFIX_.'kb_pacbz_product_zone_mapping` WHERE' .
                ' id_kb_pacbz_product_zone_mapping = "'.pSQL($product_id).'"';
            Db::getInstance()->execute($sql1);
            Tools::redirectAdmin('index.php?controller=AdminProductDetails&success_delete=1&token='.$this->token);
        }
    }
